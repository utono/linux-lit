...
pub fn ensure_voice_catalog_table(conn: &Connection) -> Result<(), rusqlite::Error> {
    ...
    // and lets a user-edited row survive a re-run. Benedick (male) / Imogen
    ...
    let seed: [(&str, &str, i64, i64, &str, &str); 8] = [
        ...
        (IMOGEN_VOICE_ID,   "female", 20, 30, "verse", "Imogen — British female verse+prose"),
        (IMOGEN_VOICE_ID,   "female", 20, 30, "prose", "Imogen — British female verse+prose"),
    ];
...
pub fn resolve_default_voice(
    ...
    // All prose (explication) is read by Imogen — one consistent narrator for
    ...
    if !is_verse {
        return (
            crate::elevenlabs::IMOGEN_VOICE_ID.to_string(),
...
mod tests {
    ...
    fn resolve_nearest_band_when_no_containment() {
        ...
        // (Prose is always Imogen — see resolve_prose_always_imogen.)
        assert_eq!(
    ...
    fn resolve_null_age_uses_default_age_40() {
        ...
        // (Juliet 12-19, Imogen 20-30); nearest is Imogen (dist 10) verse.
        assert_eq!(
            resolve_default_voice(&conn, "Rom", "NURSE", true),
            (crate::elevenlabs::IMOGEN_VOICE_ID.to_string(), crate::elevenlabs::OP_MODEL_ID.to_string())
        );
    }
    ...
    #[test]
    fn resolve_prose_always_imogen() {
        let conn = Connection::open_in_memory().unwrap();
        seed_catalog_and_chars(&conn);
        // Prose (is_verse=false) is ALWAYS Imogen, regardless of the speaker's
        // gender/age — even a young male like Romeo, or an unknown speaker.
        let imogen = (
            crate::elevenlabs::IMOGEN_VOICE_ID.to_string(),
            crate::elevenlabs::OP_MODEL_ID.to_string(),
        );
        assert_eq!(resolve_default_voice(&conn, "Lr", "LEAR", false), imogen);
        assert_eq!(resolve_default_voice(&conn, "Rom", "NOBODY", false), imogen);
