...
## Tab Keybind Behavior (All Modes)

Change Tab globally (not just during search): pressing Tab resumes playback (if paused) and seeks to `start_time - 0.2s` of the current line's timestamp. This replaces the current toggle-pause-only behavior. Works the same whether the user arrived at the line via `j`/`k`, `n`/`N`, dialogue jump, or any other navigation.
