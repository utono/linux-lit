...
- **Start-only rows fired the gap lead on every line (fixed 2026-08-16):** `b` stamped top-to-bottom left every line but the last with `end_time` NULL, and `find_line_for_time` treated an unusable `a_end` as "assume a gap", advancing 1.5s before each next start (R2-Arkangel 3.3.1-4). Signature: `TS: set start=… end=0.00` on consecutive lines, then `CURSOR_SYNC` landing at `next.start - 1.5`. Fix: `set_start_time` backfills the previous line's end (`prev_end_backfill`); with `a_end` unusable the lead needs `b.start - a.start > SYNC_ASSUMED_LINE_SPAN + SYNC_GAP_THRESHOLD`. Ledger: `docs/troubleshooting/timestamp-diagnosis.md` FAILURE MODE 8.
